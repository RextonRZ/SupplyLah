        ...
        {chatMode === "text" ? (
          ...
          <>
            <textarea
              ...
              placeholder={isRecording ? "🔴 Recording..." : "Type or hold mic…"}
              rows={1}
              disabled={isRecording}
              className="flex-1 self-end resize-none rounded-2xl border border-slate-300 px-4 py-2 text-sm leading-5 focus:outline-none focus:border-green-400 max-h-[100px] overflow-y-auto disabled:bg-slate-50 disabled:text-red-500"
            />
            {input.trim() ? (
              <button
                onClick={sendMessage}
              ...
              <button
                ...
                className={`w-10 h-10 rounded-full flex items-center justify-center transition-all shrink-0 ${
                  isRecording ? "bg-red-500 text-white" : "bg-green-600 text-white"
                } disabled:opacity-40`}
              >
